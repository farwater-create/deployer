...
export declare interface DeployerInteractionRouter {
  on<T extends Interaction>(
    event: MinecraftApplicationCustomId,
    listener: (interaction: T) => void
  ): this;
  on(
    event: "contextCommand",
    listener: (interaction: ContextMenuCommandInteraction) => void
  ): this;
  on(
    event: "command",
    listener: (interaction: CommandInteraction) => void
  ): this;
}
