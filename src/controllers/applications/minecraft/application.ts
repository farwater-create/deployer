...
export class MinecraftApplication {
    ...
    async update() {
        ...
        if (await this.offensiveSkin()) {
            ...
            (await this.user()).dmChannel
                ?.send("You have been unwhitelisted and kicked for having an offensive skin. Submit a ticket.")
                .catch(logger.error);
        }
    }
    ...
    async whitelist() {
        const {serverId, minecraftName, discordId} = this.options;
        PterodactylPanel.minecraft(serverId)
            .whitelist(minecraftName)
            .catch((err) => logger.discord("error", err));
        await linkDiscordUserToMinecraft(discordId, minecraftName);
    }

    async serialize() {
        const {discordId, minecraftUuid, reason, age, serverId, createdAt, minecraftName, minecraftSkinSum, roleId} =
            this.options;
